...
Pokemon.prototype.getPokemon = function(input) {
  ...
  $.get('http://pokeapi.co/api/v2/pokemon/' + input).then(function(response) {
    $('#display-result').html("Id -> " + response.id + "<br>Name -> " + response.forms[0].name + "<br>Ability -> " + response.abilities.ability.name + "<br>Sprite -><img src='" + response.sprites["front_default"] + "'/>");
  }).fail(function(error) {
